...
### A Simple Program
Here is a short Faust program that implements a simple noise generator (mainly using a function from the `noises.lib` Faust library). It exhibits various kind of statements: two [*global metadata*](#global-metadata) declarations, an [*imports*](#imports), a *comment*, and a [*definition*](#definitions). We will later study how [*documentation statements*](#documentation-tags) work:
...
The keyword `process` is the equivalent of `main` in C/C++. All Faust programs must have a `process` *definition*.

## Statements

Faust programs have four kinds of *statements*: 
...
All statements, except *documentation* statements, end with a semicolon `;`. 

### Metadata

Metadata allow us to add elements which are not part of the language to Faust code. Metadata usage can range from simply indicating the name of a Faust program or its author, to potential compilation options or user interface element customizations.
...
Note that some [Global Metadata](#global-metadata) have standard names and can be used for specific tasks as described in the [Standard Metadata](#standard-metadata) section.
...
allows us to specify the name of an entire Faust program. 

Unlike regular comments, metadata declarations will appear in the C++ code generated by the Faust compiler. A good practice is to start a Faust program with some common declarations:
...
This is very useful when a library has several contributors, functions potentially have different license terms, etc. 

#### Standard Metadata

There exists a special set of standard [global metadata](#global-metadata) in Faust whose role is described in the following table:

| Metadata | Role |
| :---: | --- |
| `declare options "[key0:value][key1:value]"` | This metadata can be used to indicate various options associated with a Faust program such as if it's polyphonic, if it should have OSC, if it has MIDI support, etc. Specific `keys` usable with this metadata are described throughout this documentation. |
| `declare interface "xxx"` | Specifies an interface to replace the standard Faust UI. |
...
Documentation statements are optional and are typically used to control the generation of the mathematical documentation of a Faust program. This documentation system is detailed in the [Mathematical Documentation](mathdoc.md) chapter. This section describes the documentation statement syntax.
...
Moreover, optional sub-tags can be inserted in the text content itself to generate, at the insertion point, mathematical *equations*, graphical *block-diagrams*, Faust source code *listings* and explanation *notices*.

<img src="img/equation.svg" class="mx-auto d-block">

A Faust expression's mathematical equations can be generated by placing the Faust expression between an opening `<equation>` and a closing `</equation>` tag. The expression is evaluated within the lexical context of the Faust program.

<img src="img/diagram.svg" class="mx-auto d-block">

Similarly, a Faust expression's graphical block-diagram can be generated by placing the Faust expression between an opening `<diagram>` and a closing `</diagram>` tag. The expression is evaluated within the lexical context of the Faust program.

<img src="img/metadata.svg" class="mx-auto d-block">

The `<metadata>` tags create a reference to Faust [global metadatas](#global-metadata), that has the keyword that is put between the `<metadata>` tags.

<img src="img/notice.svg" class="mx-auto d-block">

TODO ? The `<notice/>` empty-element tag is used to generate the conventions used in the mathematical equations.

<img src="img/listing.svg" class="mx-auto d-block">

TODO ? The `<listing/>` empty-element tag is used to generate the listing of the Faust program. Its three attributes `mdoctags`, `dependencies`, and `distributed` enable or disable respectively `<mdoc>` tags, other files dependencies and distribution of interleaved Faust code between `<mdoc>` sections.

## Definitions

TODO (*result* of an expression?)
A *definition* associates an identifier with an expression. Definitions are essentially a convenient shortcut to avoid typing long expressions. During compilation (in particular, during the evaluation stage) identifiers are replaced by their definitions. It is therefore always equivalent to use either an identifier or the identifier's value (expression?). Please note that multiple definitions of the same identifier are not allowed, unless it is a pattern matching based definition (more on [pattern matching](#definitions-with-pattern-matching) later).
...
This powerful mechanism allows us to algorithmically create and manipulate block diagrams expressions. Let's say that you want to describe a function to duplicate an expression several times in parallel:
...
Another example of when we might want to use pattern matching would be to count the number of elements in a list.

Please note that we simulate (?) lists using [parallel composition](#parallel-composition): `(1,2,3,5,7,11)`. The main limitation of this approach is that there is no empty list. (?) Moreover lists of only one element are represented by this element:
...
Despite its textual syntax, Faust is conceptually a block-diagram language. Faust expressions represent DSP block-diagrams and are assembled from primitive ones using various *composition* operations. More traditional *numerical* expressions that use infix notation are also possible. Additionally Faust provides time based expressions, like delays, expressions related to lexical environments, expressions to interface with foreign functions and lambda expressions.

<img src="img/expression.svg" class="mx-auto d-block">

### Constant Numerical Expressions  

Some language primitives (like `rdtable`, `rwtable`, `hslider` etc.) take constant numbers as some of their parameters. Expressions using pattern matching techniques can also incorporate constant numbers. Those numbers can be directly given in the code, or they can also be computed by more complex expressions *which must produce numbers that can be determined at compile time*. We will refer to them as [constant numerical expressions](#constant-numerical-expressions) in the documentation.

### Diagram Expressions

Diagram expressions are assembled from primitive expressions using either binary composition operations or high level iterative constructions.
...
Please note that a higher priority value means a higher priority in the evaluation order. There are companion tables that gives the associativity of each infix numerical operator in the [Infix Notation section](#infix-notation-and-other-syntax-extensions).

#### Parallel Composition

The *parallel composition operator* (e.g., `(A,B)`) is probably the simplest one. It places the two block-diagrams one on top of the other, without connections. The inputs of the resulting block-diagram are the inputs of `A` and `B`. The outputs of the resulting block-diagram are the outputs of `A` and `B`. 

*Parallel composition* is an associative operation: `(A,(B,C))` and `((A,B),C)` are equivalent. When no parenthesis are used (e.g., `A,B,C,D`), Faust uses right associativity and therefore builds internally the expression `(A,(B,(C,D)))` (? I thought it just said order doesn't matter. Does the next sentence give the exception?). TODO This organization is important to know when using pattern matching techniques on parallel compositions. 
...
Note that there's a better way to write this last example using the [`par` iteration](#par-iteration): 
...
The *sequential composition operator* (i.e., `A:B`) expects:
...
*Sequential composition* is an associative operation: `(A:(B:C))` and `((A:B):C)` are equivalent. When no parenthesis are used, like in `A:B:C:D`, Faust uses right associativity and therefore builds internally the expression `(A:(B:(C:D)))`.

**Example: Sine Oscillator**

Since everything is considered a signal generator in Faust, *sequential composition* can be simply used to pass an argument to a function:
...
*Sequential composition* can be used to create an audio effect chain. Here we're plugging a guitar distortion effect into an autowah effect:
...
The number of inputs and outputs of a Faust expression can be found at compile time simply by using `inputs(expression)` and `outputs(expression)`.

For example, the number of outputs of a sine wave oscillator can be found by writing the following program:
...
This type of construction is useful for defining higher order functions and building algorithmically complex block-diagrams. Here is an example of automatically reversing the order of the outputs of an expression.
...
For example `Xi(-)` will reverse the order of the two inputs of the substraction operator:
...
* its second argument is the total number of times to iterate, as an integer [constant numerical expression](#constant-numerical-expressions), automatically promoted to *int*.
...
Note that `i` is used here at each iteration to compute various elements and to format some labels. Having user interface elements with different names is a way to differentiate those elements in the generated interface.  
...
The following example is just a slightly different version of the example presented in the [`par` iteration](#par-iteration) section. While their block diagrams look slightly different, the generated code is exactly the same.
